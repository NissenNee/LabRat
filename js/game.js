...
const hints = {
    ...
    hint10: `*to use the command "drop": Type "drop" and the item you wamt to drop`
}
...
const txt = {
    ...
    yesTxt: {
        ...
        txt2: `Yes? What are you saying yes to? You can´t say yes to that`,
    },
    noTxt: {
        txt1:`Hmph! Alright then. If you don’t want to play, then why are you even here!? Go do something more “valuable” with your time then! ...I´m sorry! I got a little grumpy there. Come back when you’re ready to start.`,
        txt2:`No? What are you saying no to? You can´t say no to that.`,
    },
    maybeTxt: {
        txt1: `What do you mean maybe! What kind of answer is that! Are you perhaps scared? Then I guess you can just cower in fear like a pathetic lab rat! ...I apologize. I promise I’m working on my temper. Perhaps you need some instructions?`,
    ...
    gurneyTxt: {
        txt1: `The gurney looks old. It has small spots of rust on the legs and the leather mattress on top looks worn. You notice a metal piece sticking out from under the mattress.`,
    ...
    gameOverTxt: {
        ...
        txt2: `Fine! Be like that! What a sore loser. You’re no fun at all! ...Sorry! I really need to get a hold on my temper. Stopping is completely op to you. Thank you for playing.`
    },
}
...
// myFunction() {
//     alert(url(`..img/chemistryCat.jpg`)` In my professional oppinion this is a masterpiece. But it would be nice, if you would take my survey: link`);
//   }
...
const help = {
  ...
  const displayInventory = () => {
    ...
    addText(`Space Used: ${inventory.spaceUsed}/${inventory.availableSpace}`);
  };
...
const whatToDo = (input) => {
    switch (input) {
        ...
        // lav conditional statement for hvis "nametag" er ødelagt `
             /* `You look at the nametag. On the front is your name, your age and your gender. On the back is a barcode. The nametag is broken. A corner has broken off and it´s full of scratches. Some of the barcode is  missing`*/

        // lav condional statement for hvis navn, gender, age er indtastet, hvis "insertInfo" function er lavet
        ...
        // lav conditional statement, for hvis "metal piece" er taget
            /*`The gurney looks old. It has small spots of rust on the legs and the leather mattress on top looks worn.`*/
        // if(inventory.items(`metal piece`) = true){
        // addText(txt.gurneyTxt.txt2)
        // }
        // else if( hvis spilleren ikke er i metalplated room)
        // else {
        //      addText(txt.gurneyTxt.txt2)
         // }
        ...
        case 'keep metal piece':
            // lav contional statement for hvis metal piece allerede er i inventory
            if (inventory.availableSpace > 0) {
                addText(txt.keepTxt);
        ...
        break;
        case `use nametag`:
            if(txt.wallsTxt = true) {
                addtext(txt.useTxt.txt2);
            }
            else if(txt.useTxt.txt3 = true) {
                addText(txt.useTxt.txt4);
            }
            else {
                addtext(`Using nametag... What exactly are you using it for? That command isn´t currently available`);
                addText(hints.hint2);
            }
        break;
        case `use metal piece`:
            if(txt.wallsTxt = true) {
                ...
                addText(`Using metal piece... What exactly are you using it for? That command isn´t currently available`);
                addText(hints.hint2);
            }
        break;
        default:
